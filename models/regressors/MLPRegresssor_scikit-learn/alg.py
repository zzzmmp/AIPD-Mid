...
def show():
    ...
    with col1:
        with st.expander("超参数配置"):
            ...
            inputs['layer size'] = st.number_input('隐藏层数量', 1, 100, 2)
            inputs['neuron size'] = st.number_input('神经元数量', 5, 100, 10)
            inputs['hidden layer size'] = tuple([inputs['neuron size']] * inputs['layer size'])
            inputs['activation'] = st.selectbox('激活函数', ['relu', 'identity', 'logistic', 'tanh'])
            inputs['solver'] = st.selectbox('优化器', ['lbfgs', 'sgd', 'adam'])
            inputs['learning rate'] = st.selectbox('学习率策略', ['constant', 'invscaling', 'adaptive'])
            inputs['batch size'] = st.number_input('批处理大小', 4, 64, 16)
            inputs['max iter'] = st.number_input('最大迭代次数', 1, 1000, 300)
