...
def show():
    ...
    with col1:
        with st.expander("超参数配置"):
            # inputs['criterion'] = st.selectbox('分裂准则',('squared_error','friedman_mse','absolute_error','poisson'))
            ...
            if auto_hyperparameters:
                ...
                # 图形参数
        with st.expander("决策树可视化"):
            inputs['tree graph'] = st.checkbox('生成决策树图', False)
            # if inputs['tree graph']:
            #     inputs['filled'] = st.selectbox('填充颜色', ('True', 'False'))
            #     inputs['rounded'] = st.selectbox('圆角节点', ('True', 'False'))
