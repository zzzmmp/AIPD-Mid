...
def show():
    ...
    with col1:
        with st.expander("超参数配置"):
            inputs['base estimator'] = st.selectbox('基学习器类型',
                                                    ['DecisionTree', 'SupportVector', 'LinearRegression'])
            inputs['nestimators'] = st.number_input('弱学习器数量', 1, 1000, 100)
            inputs['learning rate'] = st.number_input('学习率', 0.001, 10.0, 1.0)

            random_state = st.checkbox('固定随机状态(42)', True)
            if random_state:
    ...
    # 如需独立于应用或模板测试本算法，请在本目录下执行
    # `streamlit run alg.py`
