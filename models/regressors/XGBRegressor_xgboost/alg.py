...
def show():
    ...
    with col1:
        with st.expander("超参数配置"):
            ...
            inputs['base estimator'] = st.selectbox('基学习器类型', ['gbtree', 'gblinear'])

            inputs['nestimators'] = st.number_input('弱学习器数量', 1, 10000, 100)
            inputs['learning rate'] = st.number_input('学习率', 0.01, 10.0, 0.3)
            inputs['max depth'] = st.number_input('最大树深度', 1, 100, 6)
            inputs['subsample'] = st.slider('样本采样比例', 0.5, 1.0, 1.0)
            inputs['subfeature'] = st.slider('特征采样比例', 0.5, 1.0, 1.0)
