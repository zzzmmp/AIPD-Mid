...
def model_platform(model_path):
    ...
    task = st.selectbox("模型", list(template_dict.keys()))
    if isinstance(template_dict[task], dict):
        framework = st.selectbox('平台', list(template_dict[task].keys()))
...
class customPlot:
    def __init__(self) -> None:
        ...
        self.fontsize_option = ('M', 'XS', 'S', 'L', 'XL')
        self.fontsize_dict = {
            'XS': 3,  # 特小
            'S': 5,  # 小
            'M': 8,  # 中
    ...
    def set_title_fontsize(self, key):
        ...
        # 显示代号和对应数值
        format_func = lambda x: f"{x} ({self.fontsize_dict[x]}px)"
        options_selected = st.selectbox('标题字体大小', options,
    ...
    def set_label_fontsize(self, key):
        options = self.fontsize_option
        format_func = lambda x: f"{x} ({self.fontsize_dict[x]}px)"
        options_selected = st.selectbox('坐标轴标签字体大小', options,
                                        format_func=format_func, key=key)
        return options_selected
    ...
    def set_annot_fontsize(self, key):
        options = self.fontsize_option
        format_func = lambda x: f"{x} ({self.fontsize_dict[x]}px)"
        options_selected = st.selectbox('标注字体大小', options,
                                        format_func=format_func, key=key)
        return options_selected
    ...
    def set_legend_fontsize(self, key):
        options = self.fontsize_option
        format_func = lambda x: f"{x} ({self.fontsize_dict[x]}px)"
        options_selected = st.selectbox('图例字体大小', options,
                                        format_func=format_func, key=key)
        return options_selected
    ...
    def set_tick_fontsize(self, key):
        options = self.fontsize_option
        format_func = lambda x: f"{x} ({self.fontsize_dict[x]}px)"
        options_selected = st.selectbox('标尺字体大小', options,
                                        format_func=format_func, key=key)
        return options_selected
    ...
    def feature_hist_kde(self, options_selected, feature_name, feature_value):
        ...
        fig, ax = plt.subplots()
        ax = sns.distplot(feature_value.dropna(), bins=10, kde_kws={"color": color[0], "label": "KDE", "alpha": 1},
                          hist_kws={"color": color[1], "alpha": 1, "edgecolor": "black", "linewidth": 0.5,
                                    "rwidth": 0.9})  # 增加rwidth参数
        plt.title(feature_name + " Statistics ")
        plt.legend()
    ...
    # # 修改 customPlot 类中的 pred_vs_actual 方法
    # def pred_vs_actual(self, Ytrain_actual, Ytest_actual, Ytrain_pred, Ytest_pred):
    #     with plt.style.context(['nature', 'no-latex']):
    #         fig, ax = plt.subplots()
    #
    #         # 绘制训练集点（用不同颜色和标记区分）
    #         ax.scatter(Ytrain_actual, Ytrain_pred, marker='o', s=15, color='blue',
    #                    alpha=0.6, label='训练集', zorder=1)
    #
    #         # 绘制测试集点
    #         ax.scatter(Ytest_actual, Ytest_pred, marker='s', s=20, color='red',
    #                    alpha=0.8, label='测试集', zorder=2)
    #
    #         lims = [
    #             np.min([ax.get_xlim(), ax.get_ylim()]),  # min of both axes
    #             np.max([ax.get_xlim(), ax.get_ylim()]),  # max of both axes
    ...
    #
    #         # 绘制对角线
    #         ax.plot(lims, lims, 'k-', zorder=3, linewidth=1.5, linestyle='solid', color='#000000')
    #         ax.set_xlim(lims)
    #         ax.set_ylim(lims)
    #
    #         plt.xlabel("实际值 (Actual)")
    #         plt.ylabel("预测值 (Prediction)")
    #         plt.legend()
    #         st.pyplot(fig)

    def pred_vs_actual_1(self, actual, pred):
        # 设置中文字体支持
        plt.rcParams['font.sans-serif'] = ['SimHei', 'DejaVu Sans']  # 黑体优先，英文回退
        ...
        with plt.style.context(['nature', 'no-latex']):
            ...
            # 美化散点图
            ax.scatter(actual, pred, marker='o', s=40, color='#2E86AB', alpha=0.7,
                       edgecolors='#2E86AB', linewidth=1.2, zorder=3)
            ...
            ax.plot(lims, lims, 'k--', zorder=2, linewidth=2, color='#FF6B6B',
                    alpha=0.8, label='Perfect Prediction')
            ax.tick_params(direction='in', length=6, width=1, labelsize=10, colors='#333333')
            plt.xlabel("Actual Value", fontsize=12, fontweight='bold', color='#333333')
            plt.ylabel("Predicted Value", fontsize=12, fontweight='bold', color='#333333')
            plt.title(f"Predicted vs Actual (R² = {r2:.4f})", fontsize=14, fontweight='bold', pad=20)
    ...
    def pred_vs_actual(self, Ytrain_actual, Ytest_actual, Ytrain_pred, Ytest_pred):
        ...
        with plt.style.context(['nature', 'no-latex']):
            ...
            # 绘制训练集散点图
            ax.scatter(Ytrain_actual, Ytrain_pred, marker='o', s=40, color='#2E86AB', alpha=0.6,
                       edgecolors='#2E86AB', linewidth=1.0, zorder=2, label=f'Train (R² = {train_r2:.4f})')

            # 绘制测试集散点图
            ax.scatter(Ytest_actual, Ytest_pred, marker='s', s=50, color='#FF6B6B', alpha=0.8,
                       edgecolors='#FF6B6B', linewidth=1.5, zorder=3, label=f'Test (R² = {test_r2:.4f})')

            # 设置坐标轴范围
            all_actual = np.concatenate([Ytrain_actual, Ytest_actual])
            all_pred = np.concatenate([Ytrain_pred, Ytest_pred])
            ...
            # 绘制完美预测对角线
            ax.plot(lims, lims, 'k--', zorder=1, linewidth=2, color='#333333',
                    alpha=0.7, label='Perfect Prediction')

            ax.tick_params(direction='in', length=6, width=1, labelsize=10, colors='#333333')
            plt.xlabel("Actual Value", fontsize=12, fontweight='bold', color='#333333')
            plt.ylabel("Predicted Value", fontsize=12, fontweight='bold', color='#333333')
            plt.title("Predicted vs Actual Values", fontsize=14, fontweight='bold', pad=20)
            ...
            # st.pyplot(fig)
            # st.pyplot(fig, use_container_width=True)  # 关键参数
            # Plotly会自动响应缩放
...
class FeatureSelector:
    ...
    def identify_zero_low_importance(self, cumulative_importance=0.95):
        ...
        st.write('为了达到 %.2f 的累积重要性，需要保留前 %d 个最重要的特征。' % (self.cumulative_importance,
                                                                               len(self.feature_importances) - len(
                                                                                   self.record_low_importance)))

    def feature_importance_select_show(self):
        st.write('---')
        st.write(self.feature_importances)
        tmp_download_link = download_button(self.feature_importances, f'importance rank.csv', button_text='下载')
        ...
        with st.expander('**特征数据列表**'):
            col1, col2 = st.columns([1, 1])
            with col1:
                st.write('零重要性特征删除列表')
                st.write(self.features_dropped_zero_importance)
                tmp_download_link = download_button(self.features_dropped_zero_importance, f'零重要性特征删除列表.csv',
                                                    button_text='下载')
                st.markdown(tmp_download_link, unsafe_allow_html=True)
            with col2:
                st.write('高重要性特征列表')
                st.write(self.features_dropped_low_importance)
                tmp_download_link = download_button(self.features_dropped_low_importance, f'高重要性特征列表.csv',
                                                    button_text='下载')
                st.markdown(tmp_download_link, unsafe_allow_html=True)
...
class REGRESSOR:
    ...
    def DecisionTreeRegressor(self):
        ...
        self.model.fit(self.Xtrain, self.Ytrain)
        self.Ypred = self.model.predict(self.Xtest)
        self.Ytrain_pred = self.model.predict(self.Xtrain)  # 训练集
...
#     with st.expander("模型文件"):
#         tmp_download_link = download_button(model.model, model_name+'_model.pickle', button_text='下载')
#         st.markdown(tmp_download_link, unsafe_allow_html=True)
#     with st.expander('预测结果'):
#         st.write(result_data)
#         tmp_download_link = download_button(result_data, f'{model_name}_预测结果.csv', button_text='下载')
#         st.markdown(tmp_download_link, unsafe_allow_html=True)
...
def plot_and_export_results(model, model_name):
    plot = customPlot()
    # 同时绘制训练集和测试集
    plot.pred_vs_actual(model.Ytrain, model.Ytest, model.Ytrain_pred, model.Ypred)

    # 合并测试集结果
    test_result_data = pd.concat([model.Ytest, pd.DataFrame(model.Ypred)], axis=1)
    test_result_data.columns = ['actual', 'prediction']

    # 合并训练集结果（如果有训练集预测值）
    train_result_data = pd.concat([model.Ytrain, pd.DataFrame(model.Ytrain_pred)], axis=1)
    train_result_data.columns = ['actual', 'prediction']

    with st.expander("模型文件"):
        tmp_download_link = download_button(model.model, model_name + '_model.pickle', button_text='下载')
        st.markdown(tmp_download_link, unsafe_allow_html=True)

    with st.expander('测试集预测结果'):
        st.write(test_result_data)
        tmp_download_link = download_button(test_result_data, f'{model_name}_测试集预测结果.csv', button_text='下载')
        st.markdown(tmp_download_link, unsafe_allow_html=True)

    with st.expander('训练集预测结果'):
        st.write(train_result_data)
        tmp_download_link = download_button(train_result_data, f'{model_name}_训练集预测结果.csv', button_text='下载')
        st.markdown(tmp_download_link, unsafe_allow_html=True)
...
def plot_and_export_results_clf(model, model_name, col_name, unique_categories):
    ...
    with st.expander('Actual vs Predict'):
        st.write(result_data)
        tmp_download_link = download_button(result_data, f'actual vs prediction.csv', button_text='下载')
        st.markdown(tmp_download_link, unsafe_allow_html=True)       
...
def export_cross_val_results(model, F, model_name, random_state):
    ...
    plot = customPlot()
    plot.pred_vs_actual_1(Y_test, Y_pred)
    with st.expander("模型文件"):
        tmp_download_link = download_button(model.model, model_name+'_model.pickle', button_text='下载')
        st.markdown(tmp_download_link, unsafe_allow_html=True)
...
def export_cross_val_results_clf(model, F, model_name, col_name, unique_categories, random_state):
    ...
    with st.expander("model"):
        tmp_download_link = download_button(model.model, model_name+'.pickle', button_text='下载')
        st.markdown(tmp_download_link, unsafe_allow_html=True)
    ...
    with st.expander('Actual vs Predict'):
        st.write(result_data)
        tmp_download_link = download_button(result_data, f'prediction.csv', button_text='下载')
        st.markdown(tmp_download_link, unsafe_allow_html=True)
...
def feature_transform(df):
    ...
    # 第一步：化学式转换
    status_text.text("步骤 1/2: 转换化学式为成分对象...")
    df = StrToComposition().featurize_dataframe(df, "Pretty_formula")
    progress_bar.progress(30)
    time.sleep(0.5)  # 让进度条更平滑

    # 第二步：特征计算
    status_text.text("步骤 2/2: 计算 MAGPIE 特征...")
    EP = ElementProperty.from_preset(preset_name="magpie")
    ...
    for idx, row in tqdm(df.iterrows(), total=total_rows, desc="处理中"):
        try:
            features = EP.featurize(row['composition'])
            feature_data.append(features)
        except Exception as e:
            # 出错时填充空值
            feature_data.append([None] * len(EP.feature_labels()))

        # 更新Streamlit进度条
        progress = 30 + (idx + 1) / total_rows * 70
        progress_bar.progress(int(progress))
        status_text.text(f"处理中: {idx + 1}/{total_rows} 行完成")

    # 添加特征列
    feature_columns = EP.feature_labels()
    for i, col_name in enumerate(feature_columns):
        df[col_name] = [feat[i] for feat in feature_data]
